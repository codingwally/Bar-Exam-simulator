-- Add a purpose-built, audited source for the Executive Pulse "Recent Users"
-- table. The existing paginated user directory is ordered for directory
-- browsing, so it cannot reliably identify the most recent sign-ins once the
-- account population exceeds one page.

begin;

create or replace function public.admin_recent_sign_in_directory(
...
begin
  ...
  if exists (
    ...
    where a.actor_user_id = p_actor_user_id
      ...
      and a.target_resource_type = 'admin_recent_sign_in_directory'
  ...
  from (
    ...
    left join public.admin_user_answer_counts() counts on counts.user_id = u.id
  ...
  if not exists (
    ...
    insert into public.admin_audit_log (
      ...
      'admin_recent_sign_in_directory',
      'executive_pulse',
...
revoke all on function public.admin_recent_sign_in_directory(
  uuid, integer, text
) from public, anon, authenticated;
grant execute on function public.admin_recent_sign_in_directory(
  uuid, integer, text
) to service_role;

comment on function public.admin_recent_sign_in_directory(
  uuid, integer, text
) is 'Capability-restricted and audited recent sign-in directory with exact email plus privacy-safe coarse device and region metadata.';
