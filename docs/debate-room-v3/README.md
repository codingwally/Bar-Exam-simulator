...
The owner explicitly authorized public access for every signed-in member, paid or unpaid, with a separate Debate Room link beside Study Room. The earlier admin-only preview interpretation is superseded. Publication and full acceptance remain distinct: the owner requested access while implementation continues. [Publication requirements and current evidence](publication-20260910.md) track the public entry, professional copy, desktop layout and remaining rehearsal gaps. [Production database evidence](evidence/production-database-20260910.json) verifies both additive migrations, preserved Study records, rollback-only checks, permissions and private bucket configuration. [Exact `af963655` Worker and Pages publication is recorded](evidence/publication-34404153144.json), including 29 live asset hashes. [The existing owner Chrome session passed ordinary reload, the 44-second empty-lobby check, rules guide and setup cancellation](evidence/live-browser-copy-af96365.json), without creating an event. This resolves the [earlier `44451be` script-cache failure](evidence/live-browser-cache-gap-44451be.json) at that observed scope. Copy/layout source and controlled-browser checks passed; full 93-minute hosted rehearsal, private files, physical media, real mail, separate paid/unpaid journeys, Study background-refresh interaction and the whole-site audit remain open.
