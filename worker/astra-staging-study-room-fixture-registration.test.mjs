...
const migrationPath = 'supabase/migrations/20260908131116_astra_staging_study_room_fixture_registration.sql';
...
const identity = (label='student') => ({label,runId:RUN,
  email:`dd-study-room-${label}-${RUN}@example.com`,
  fullName:`Synthetic Study Room ${label}`,source:`astra_study_room_staging_v1:${RUN}:${label}`});
...
async function seed(db,who=identity()) {
  await db.query('insert into auth.users(id,email,raw_app_meta_data,raw_user_meta_data) values($1,$2,$3,$4)',
    [ID,who.email,{provider:'email',providers:['email'],astra_staging_study_room_fixture:{version:1,runId:who.runId,label:who.label}},
...
async function register(db,{id=ID,runId=RUN,label='student',schema='public',role='service_role'}={}) {
  assert.ok(['public','private'].includes(schema));
  return (await call(db,`select ${schema}.astra_register_staging_study_room_fixture($1::uuid,$2::text,$3::text) v`,
...
test('optional Study Room registrar uses actual SQL, preserves account state, and fails closed',async t=>{
  ...
  try {
    await t.test('service-only invoker wrapper/private definer and postgres-only registry',async()=>{
      const rows=(await db.query(`select n.nspname,p.prosecdef,p.proconfig,pg_get_userbyid(p.proowner) owner,
        ...
        where p.proname='astra_register_staging_study_room_fixture' order by n.nspname`)).rows;
    ...
    for(const who of [identity('student'),identity('primary-admin'),identity('secondary-admin'),identity('founder-admin')]) {
      await t.test(`${who.label}: register as student before later explicit test setup, preserve exact replay`,()=>fixtureCase(db,async()=>{
        ...
        assert.deepEqual(result,{registered:true,fixtureUserId:ID,dataScope:'internal_test',registrationVersion:'astra-staging-study-room-v1',replayed:false});
        ...
        if(who.label==='student') await db.exec("insert into public.free_beta_access(user_id) values('10000000-0000-4000-8000-000000000001')");
        else await db.query('update public.user_roles set role=$1 where user_id=$2',[who.label==='founder-admin'?'founder_admin':'admin',ID]);
    ...
    const negatives=[
      ...
      ['NULL label',null,{label:null}],['unknown label',null,{label:'owner'}],['wrong allowed label',null,{label:'primary-admin'}],
      ['Examination label',null,{label:'student-a'}],['admin label',null,{label:'admin'}],
      ['no trusted marker',"update auth.users set raw_app_meta_data=raw_app_meta_data-'astra_staging_study_room_fixture'"],
      ['editable marker only',"update auth.users set raw_user_meta_data=raw_user_meta_data||jsonb_build_object('astra_staging_study_room_fixture',raw_app_meta_data->'astra_staging_study_room_fixture'),raw_app_meta_data=raw_app_meta_data-'astra_staging_study_room_fixture'"],
      ...[['version','2'],['runId','"different"'],['label','"admin"']].map(([key,value])=>[`different marker ${key}`,`update auth.users set raw_app_meta_data=jsonb_set(raw_app_meta_data,'{astra_staging_study_room_fixture,${key}}','${value}')`]),
      ['extra marker property',"update auth.users set raw_app_meta_data=jsonb_set(raw_app_meta_data,'{astra_staging_study_room_fixture,extra}','true')"],
    ...
    for(const [name,sql,args] of negatives) await t.test(`deny ${name} before any registry write`,()=>fixtureCase(db,async()=>{
      ...
      await assert.rejects(()=>register(db,args),error=>error.code==='P0001'&&/^Staging Study Room fixture /.test(error.message));
    ...
    for(const [name,change] of [
      ['unknown overload',"create function public.astra_register_staging_study_room_fixture(uuid) returns jsonb language sql as $$ select '{}'::jsonb $$"],
      ['private body',"create or replace function private.astra_register_staging_study_room_fixture(p_user_id uuid,p_run_id text,p_label text) returns jsonb language sql security definer set search_path='' as $$ select '{}'::jsonb $$"],
      ['public ACL','grant execute on function public.astra_register_staging_study_room_fixture(uuid,text,text) to authenticated'],
      ['wrapper volatility','alter function public.astra_register_staging_study_room_fixture(uuid,text,text) stable'],
      ['wrapper strict NULL bypass','alter function public.astra_register_staging_study_room_fixture(uuid,text,text) strict'],
      ['private leakproof','alter function private.astra_register_staging_study_room_fixture(uuid,text,text) leakproof'],
      ['wrapper parallel safety','alter function public.astra_register_staging_study_room_fixture(uuid,text,text) parallel safe'],
      ...
      await db.exec('begin');try{
        ...
        const sql="select n.nspname,pg_get_functiondef(p.oid) body,p.proacl::text acl from pg_proc p join pg_namespace n on n.oid=p.pronamespace where p.proname='astra_register_staging_study_room_fixture' order by n.nspname";
        const before=(await db.query(sql)).rows;await db.exec('savepoint migration_attempt');
        await assert.rejects(()=>db.exec(migration),/Staging Study Room (registration (function collision or drift|prerequisite missing)|registry protection changed)/);
...
test('optional migration does not modify Forecast or Examination registration, product access or registry table grants',async()=>{
  ...
  assert.doesNotMatch(migration,/astra_register_staging_(?:forecast|examination|commercial)_fixture|grant\s+(?:all|insert|select|update|usage)\b/i);
