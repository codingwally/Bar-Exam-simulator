...
begin;
do $install_payment_registrar$
declare
  v_target record;
  v_oid oid;
  v_body text;
  v_private text := $private_sql$
create or replace function private.astra_register_staging_payment_fixture(
...
begin
  if p_user_id is null or p_run_id is null or p_label is null then
    raise exception 'Staging complete-beta fixture identity is invalid';
  end if;
  if p_run_id ~ '^[a-z0-9]{8,12}-[a-f0-9]{8}$' and p_label in ('student','peer') then
    v_email := 'dd-complete-beta-' || p_label || '-' || p_run_id || '@duediligence.ph';
    v_name := 'Release ' || p_label;
  else
    raise exception 'Staging complete-beta fixture identity is invalid';
  end if;
  v_source := 'astra_complete_beta_staging_v1:' || p_run_id || ':' || p_label;
  ...
  if not found then raise exception 'Staging complete-beta fixture Auth account is missing'; end if;
  if v_user.email is distinct from v_email
     ...
     or v_user.raw_app_meta_data->'astra_staging_complete_beta_fixture' is distinct from
     ...
     or v_user.raw_app_meta_data - array['provider','providers','astra_staging_complete_beta_fixture'] is distinct from '{}'::jsonb
    ...
    raise exception 'Staging complete-beta fixture trusted identity does not match';
  end if;
  ...
  if v_role is distinct from 'student'
    ...
    raise exception 'Staging complete-beta fixture must have no privileged or billing state';
  end if;
  if v_user.last_sign_in_at is not null
    ...
    raise exception 'Staging complete-beta fixture registration must precede sign-in';
  end if;
  ...
  if found then
    if v_classification.email_at_classification is distinct from v_email
      ...
      raise exception 'Staging complete-beta fixture classification conflicts with existing evidence';
    end if;
    return jsonb_build_object('registered',true,'fixtureUserId',p_user_id,'dataScope','internal_test',
      'registrationVersion','astra-staging-complete-beta-v1','replayed',true);
  end if;
  ...
  return jsonb_build_object('registered',true,'fixtureUserId',p_user_id,'dataScope','internal_test',
    'registrationVersion','astra-staging-complete-beta-v1','replayed',false);
end;
$function$;
$private_sql$;
  v_public text := $public_sql$
create or replace function public.astra_register_staging_payment_fixture(
  p_user_id uuid, p_run_id text, p_label text
)
returns jsonb language sql volatile security invoker set search_path = ''
as $function$
  select private.astra_register_staging_payment_fixture(p_user_id,p_run_id,p_label);
$function$;
$public_sql$;
begin
  if current_user <> 'postgres' or to_regclass('private.internal_test_accounts') is null
    ...
    raise exception 'Staging complete-beta registration prerequisite missing';
  end if;
  if exists(select 1 from unnest(array['anon','authenticated','service_role']) g(role_name)
    ...
    raise exception 'Staging complete-beta registry protection changed';
  end if;
  ...
  for v_target in select * from (values
    ('private.astra_register_staging_payment_fixture(uuid,text,text)',true,'plpgsql',v_private),
    ('public.astra_register_staging_payment_fixture(uuid,text,text)',false,'sql',v_public)
    ...
    if v_oid is not null and not exists(select 1 from pg_proc p join pg_language l on l.oid=p.prolang
      ...
      raise exception 'Staging complete-beta registration function collision or drift';
    end if;
  end loop;
  execute v_private;
  execute v_public;
  revoke all on function private.astra_register_staging_payment_fixture(uuid,text,text) from public,anon,authenticated,service_role;
  revoke all on function public.astra_register_staging_payment_fixture(uuid,text,text) from public,anon,authenticated,service_role;
  grant execute on function private.astra_register_staging_payment_fixture(uuid,text,text) to service_role;
  grant execute on function public.astra_register_staging_payment_fixture(uuid,text,text) to service_role;
end;
$install_payment_registrar$;
commit;
