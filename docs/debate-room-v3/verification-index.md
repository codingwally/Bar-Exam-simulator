...
Current publication, 10 September 2026 (Asia/Manila): [exact revision `44451be` was published](evidence/publication-34401062163.json) through both Workers and Pages, with 29 live asset hashes and public authorization checks. Its copy and desktop/responsive layout passed the recorded source, controlled-browser and pixel checks. [An actual returning Chrome session still loaded old room scripts from disk cache](evidence/live-browser-cache-gap-44451be.json) and displayed the empty-lobby timer error. Published code therefore does not establish corrected behavior for returning visitors. [PR 371](https://github.com/codingwally/Bar-Exam-simulator/pull/371) candidate `de070df` passed [isolated CI 34402952395](https://github.com/codingwally/Bar-Exam-simulator/actions/runs/34402952395); its cache-refresh correction is not yet deployed. See [current publication requirements and remaining checks](publication-20260910.md). Full acceptance remains open, including the 93-minute hosted rehearsal, complete three-judge flow, private upload/download, physical media/capacity, real mail and separate production paid/unpaid journeys.
...
- **Deployment/publication:**later exact `44451be` publication through both Workers and Pages passed the checks recorded in the [current release receipt](evidence/publication-34401062163.json). Public access for signed-in members is authorized; separate real paid/unpaid journeys and corrected returning-browser cache behavior remain unverified. Full hosted flow, deployed rollback and full acceptance remain open. Earlier restricted deployment failures remain historical evidence.

Historical pre-application inventory, local adapted installation rollback, failed browser/OOM/cron/attendance runs and reviewed synthetic samples remain traceable. Their deployment and failed-smoke statuses describe their original checkpoints. Later staging and public publication are recorded separately above, including the current returning-browser failure. No provider media connection, real mail, recording, upgrade or new spending is claimed. The superior outage/pricing task's recovery assets, DNS and backups remain protected.
