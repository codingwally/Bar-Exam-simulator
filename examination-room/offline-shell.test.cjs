...
test('student registers the Examination Room service worker without blocking startup', () => {
  assert.match(studentScript, /serviceWorker\.register\('\/service-worker\.js\?v=examination-room-reliability-20260828-1'\)/);
...
test('service worker precaches the complete local student examination shell', () => {
  ...
  assert.ok(studentHtml.includes('student.js?v=reliability-20260828-1'));
  assert.ok(studentHtml.includes('api.js?v=reliability-20260828-1'));
  assert.ok(studentHtml.includes('media-capture.js?v=reliability-20260828-1'));
