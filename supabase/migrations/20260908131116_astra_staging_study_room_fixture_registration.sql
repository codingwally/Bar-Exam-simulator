...
-- an identity proof. The existing Forecast/Examination registrars and audit schema are unchanged.
begin;
do $install_study_room_registrar$
declare
  v_target record;
  v_oid oid;
  v_body text;
  v_private text := $private_sql$
create or replace function private.astra_register_staging_study_room_fixture(
  p_user_id uuid, p_run_id text, p_label text
)
...
declare
  ...
  v_role text;
begin
  if p_user_id is null or p_run_id is null or p_label is null then
    raise exception 'Staging Study Room fixture identity is invalid';
  end if;
  if p_run_id ~ '^[a-z0-9]{8,12}-[a-f0-9]{8}$' and p_label in ('student','primary-admin','secondary-admin','founder-admin') then
    v_email := 'dd-study-room-' || p_label || '-' || p_run_id || '@example.com';
    v_name := 'Synthetic Study Room ' || p_label;
  else
    raise exception 'Staging Study Room fixture identity is invalid';
  end if;
  v_source := 'astra_study_room_staging_v1:' || p_run_id || ':' || p_label;
  ...
  if not found then raise exception 'Staging Study Room fixture Auth account is missing'; end if;
  if v_user.email is distinct from v_email
     ...
     or v_user.raw_app_meta_data->'astra_staging_study_room_fixture' is distinct from
       jsonb_build_object('version',1,'runId',p_run_id,'label',p_label)
     ...
     or v_user.raw_app_meta_data - array['provider','providers','astra_staging_study_room_fixture'] is distinct from '{}'::jsonb
    ...
    raise exception 'Staging Study Room fixture trusted identity does not match';
  end if;
  ...
  if v_role is distinct from 'student'
    ...
    raise exception 'Staging Study Room fixture must have no privileged or billing state';
  end if;
  if v_user.last_sign_in_at is not null
    ...
    raise exception 'Staging Study Room fixture registration must precede sign-in';
  end if;
  ...
  if found then
    if v_classification.email_at_classification is distinct from v_email
      ...
      raise exception 'Staging Study Room fixture classification conflicts with existing evidence';
    end if;
    return jsonb_build_object('registered',true,'fixtureUserId',p_user_id,'dataScope','internal_test',
      'registrationVersion','astra-staging-study-room-v1','replayed',true);
  end if;
  ...
  return jsonb_build_object('registered',true,'fixtureUserId',p_user_id,'dataScope','internal_test',
    'registrationVersion','astra-staging-study-room-v1','replayed',false);
end;
$function$;
$private_sql$;
  v_public text := $public_sql$
create or replace function public.astra_register_staging_study_room_fixture(
  p_user_id uuid, p_run_id text, p_label text
)
returns jsonb language sql volatile security invoker set search_path = ''
as $function$
  select private.astra_register_staging_study_room_fixture(p_user_id,p_run_id,p_label);
$function$;
$public_sql$;
begin
  if current_user <> 'postgres' or to_regclass('private.internal_test_accounts') is null
    ...
    raise exception 'Staging Study Room registration prerequisite missing';
  end if;
  if exists(select 1 from unnest(array['anon','authenticated','service_role']) g(role_name)
    ...
    raise exception 'Staging Study Room registry protection changed';
  end if;
  ...
  for v_target in select * from (values
    ('private.astra_register_staging_study_room_fixture(uuid,text,text)',true,'plpgsql',v_private),
    ('public.astra_register_staging_study_room_fixture(uuid,text,text)',false,'sql',v_public)
  ) t(signature,definer,language_name,definition) loop
    v_oid := to_regprocedure(v_target.signature);
    if (select count(*) from pg_proc where pronamespace=split_part(v_target.signature,'.',1)::regnamespace
        and proname='astra_register_staging_study_room_fixture') <> (case when v_oid is null then 0 else 1 end) then
      raise exception 'Staging Study Room registration function collision or drift';
    end if;
    ...
    if v_oid is not null and not exists(select 1 from pg_proc p join pg_language l on l.oid=p.prolang
      ...
      and p.proargnames=array['p_user_id','p_run_id','p_label']
  ...
  execute v_private;
  execute v_public;
  revoke all on function private.astra_register_staging_study_room_fixture(uuid,text,text) from public,anon,authenticated,service_role;
  revoke all on function public.astra_register_staging_study_room_fixture(uuid,text,text) from public,anon,authenticated,service_role;
  grant execute on function private.astra_register_staging_study_room_fixture(uuid,text,text) to service_role;
  grant execute on function public.astra_register_staging_study_room_fixture(uuid,text,text) to service_role;
end;
$install_study_room_registrar$;
commit;
