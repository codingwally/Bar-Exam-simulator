-- Tighten historical examination access so every history-only authorization
-- is bound either to one owned attempt or to one explicitly requested track.
-- A missing track never grants unscoped access merely because another attempt
-- exists. This follows the staging-first restoration in 20260827113000.
...
comment on function public.examination_authorize_access(uuid, text, uuid, uuid, boolean)
is 'Worker-only examination access resolver. Historical access requires an owned attempt or an explicitly requested track with an owned attempt in that same track.';
