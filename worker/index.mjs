...
const EXAMINATION_ROOM_RECOVERY_REFERENCE_PREFIX = 'r2:EXAMINATION_ROOM_BACKUPS:';
...
async function examinationRoomOwnerPreflight(env) {
  ...
  try {
    ...
    checks.push({
      ...
      ok: true,
      status: 'ready',
      message: 'The backup master key and private R2 bucket are ready.',
    ...
    checks.push({
      ...
      recovery: String(error?.recovery || 'Configure the backup master key and R2 bucket, then run Preflight again.'),
    });
  }
...
function examinationRoomSnapshotDescriptor(snapshot) {
  ...
  if (!reference.startsWith(EXAMINATION_ROOM_RECOVERY_REFERENCE_PREFIX)) {
    throw examinationRoomOwnerError(
  ...
  return {
    objectKey: reference.slice(EXAMINATION_ROOM_RECOVERY_REFERENCE_PREFIX.length),
    snapshotSha256,
  };
}
