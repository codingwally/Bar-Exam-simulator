...
Source base: `7cf505dea809fbb8f35d6c88171fba7b62bf52cd`. This change adds isolated primitives and tests. It does not modify any Study or Debate issuer, room, route, workflow, provider setting, credential, existing table, paid plan or deployed database. F01/F02 acceptance is not established.
...
- `scripts/test-media-capacity-sql.mjs`: exact migration in a dedicated native PostgreSQL17 CI database, with real independent connections contending for the shared last seat. This runner is not yet wired into a workflow.
