...
3. Apply the additive migration to a production-shaped staging database.
...
| Optional student access code | Frozen in the immutable publication. When off, no student secret is generated or displayed; rostered account and admission checks remain mandatory. When on, a code is an additional barrier only. Preflight must receive an explicit boolean policy from the server; a missing/unknown value blocks start rather than being treated as roster-only. |
...
- authenticated staging verification of both optional-code modes, including no secret creation/display in roster-only mode and wrong/missing/correct code behavior in additional-code mode;
