...
test('student registers the Examination Room service worker without blocking startup', () => {
  assert.match(studentScript, /serviceWorker\.register\('\/service-worker\.js\?v=examination-room-v1-20260826-2'\)/);
...
test('service worker precaches the complete local student examination shell', () => {
  ...
  assert.ok(studentHtml.includes('student.js?v=greenfield-v1-20260826-5'));
  assert.ok(studentHtml.includes('api.js?v=greenfield-v1-20260826-7'));
