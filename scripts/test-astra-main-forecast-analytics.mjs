...
import { analyticsFixture } from '../worker/forecast-analytics-test-fixture.mjs';
...
const canonicalFixture = await analyticsFixture(2);
...
function fixture() {
  ...
  const context = vm.createContext({ window, document, console, AbortController, Intl, URL, location, history, Element: Node,
    Blob, ArrayBuffer, Uint8Array, structuredClone, CustomEvent: class { constructor(type, options) { this.type = type; this.detail = options?.detail; } } });
...
function periodFixture({ stall = '', badHash = false, badWorker = false } = {}) {
  const c = fixture(); const data = structuredClone(canonicalFixture); data.scope.id = SCOPE_ID;
  const workers = []; const messages = [];
  c.window.Worker = class {
    ...
    postMessage(message) {
      ...
      this.last = message;
      if (message.type === stall) return;
      this.reply(message);
    }
    reply(message = this.last) {
      const base = { requestId: 1, scopeId: data.scope.id, scopeHash: data.scope.scopeHash };
      let response = message.type === 'start' ? { ...base, type: 'ready', totalAttempts: data.scope.manifest.length, pdfVersion: 'forecast-analytics-pdf-v1' }
        : message.type === 'append' ? { ...base, type: 'progress', completedAttempts: message.index + 1, totalAttempts: data.scope.manifest.length, pageCount: 1 + 21 * (message.index + 1) }
        : { ...base, type: 'result', completedAttempts: data.scope.manifest.length, pdfVersion: 'forecast-analytics-pdf-v1',
          fileName: `duediligence-forecast-analytics-${SCOPE_ID}-v1.pdf`, bytes: new TextEncoder().encode('%PDF-inert-serial-ui-fixture').buffer };
      if (badWorker) response = { ...response, scopeHash: 'b'.repeat(64) };
      // Deliberately ignores terminate() to prove the UI cannot accept a late reply.
      this.onmessage?.({ data: response });
    }
  };
  c.transport(body => {
    if (body.operation === 'history') return payload({ attempts: data.attempts, analytics: data.scope.analytics });
    if (['analytics_snapshot', 'analytics_report'].includes(body.operation)) return { scope: data.scope, email: { status: 'not_requested' } };
    if (body.operation === 'analytics_attempt') {
      const index = data.scope.manifest.findIndex(row => row.attemptId === body.attemptId);
      return { scopeId: SCOPE_ID, scopeHash: data.scope.scopeHash,
        resultHash: badHash ? 'b'.repeat(64) : data.scope.manifest[index].resultHash, attempt: data.attempts[index] };
    }
    if (body.operation === 'analytics_pdf_prepared') return { ok: true, recorded: true, clientReported: true };
    throw new Error('Unexpected inert operation');
  });
  return { ...c, data, workers, messages };
}
function installShowPage(c) {
  c.nodes.set('page-mock', new Node('div')); c.window.scrollTo = () => {};
  Object.assign(c.location, { pathname: '/', search: '' });
  c.history.pushState = function(state, _title, href) {
    this.state = state; const url = new URL(href, c.location.href);
    Object.assign(c.location, { href: url.href, pathname: url.pathname, search: url.search, hash: url.hash });
  };
  Object.assign(c.context, { examStage: 'answering', examPausedForPage: false,
    cancelPremiumNavigationIntent() {}, setSiteMenuOpen() {}, activatePrimaryMenuItem() {},
    SPA_PAGE_TABS: {}, SPA_PAGE_HASHES: { mock: '#mock' } });
  const start = html.indexOf('function showPage('), end = html.indexOf("window.addEventListener('popstate'", start);
  vm.runInContext(html.slice(start, end) + ';window.__showPage=showPage;', c.context);
  return c.window.__showPage;
}

test('period PDF reads each frozen member serially, sends no token to the worker and never calls server PDF', async () => {
  const c = periodFixture(); c.state.answers.set('private-draft', 'Keep me');
  const before = JSON.stringify(c.data); await c.mount().refresh();
  for (let index = 0; index < 2; index++) {
    await button(c.host, 'Download period PDF').click(); await flush();
    assert.equal(c.objectUrls.length, index + 1); assert.equal(c.workers[index].terminated, true);
  }
  assert.deepEqual(c.messages.map(row => row.type), ['start', 'append', 'append', 'finish', 'start', 'append', 'append', 'finish']);
  assert.deepEqual(c.messages.filter(row => row.type === 'append').map(row => row.index), [0, 1, 0, 1]);
  ...
  assert.match(c.host.textContent, /PDF download started/);
});

test('ordinary pushState navigation cancels period preparation before any late download', async () => {
  const c = periodFixture({ stall: 'finish' }); const main = installMain(c); const show = installShowPage(c);
  await main.selectMainAnalyticsTab('forecast');
  const download = button(c.host, 'Download period PDF').click(); await flush();
  assert.equal(c.workers[0].last.type, 'finish'); assert.equal(show('mock'), true);
  await download; c.workers[0].reply(); await flush();
  assert.equal(c.location.hash, '#mock'); assert.equal(c.workers[0].terminated, true); assert.equal(c.objectUrls.length, 0);
  assert.equal(c.calls.some(row => row.body.operation === 'analytics_pdf_prepared'), false);
});

test('ordinary departure also invalidates an in-flight lazy Analytics module mount', async () => {
  const c = fixture(); const pending = deferred(); c.window.DueDiligenceFeatureLoader.loadForFeature = () => pending.promise;
  const main = installMain(c); const show = installShowPage(c); const opening = main.selectMainAnalyticsTab('forecast');
  show('mock'); pending.resolve(true); await opening;
  assert.equal(main.mainAnalyticsTabs.forecast, null); assert.equal(c.calls.length, 0);
});

test('cancel and owner A→B→A invalidate old period work without a stale blob or note', async () => {
  for (const mode of ['cancel', 'owner']) {
    const c = periodFixture({ stall: 'finish' }); await c.mount().refresh();
    const download = button(c.host, 'Download period PDF').click(); await flush();
    if (mode === 'cancel') await button(c.host, 'Cancel').click();
    else { c.session.user.id = OTHER; c.emit('duediligence:session'); c.session.user.id = OWNER; c.emit('duediligence:session'); }
    await download; c.workers[0].reply(); await flush();
    assert.equal(c.workers[0].terminated, true); assert.equal(c.objectUrls.length, 0);
    assert.equal(c.calls.some(row => row.body.operation === 'analytics_pdf_prepared'), false);
  }
});

test('step timeout ends an unresponsive period worker and rejects a late result', async () => {
  const c = periodFixture({ stall: 'finish' }); await c.mount().refresh();
  const download = button(c.host, 'Download period PDF').click(); await flush();
  const timer = [...c.timers.values()].find(row => row.delay === 60000); assert.ok(timer); timer.fn(); await download;
  c.workers[0].reply(); await flush();
  assert.match(c.host.textContent, /timed out/); assert.equal(c.objectUrls.length, 0);
  assert.equal(c.timers.size, 0); assert.equal(c.workers[0].terminated, true);
});

test('mismatched member hashes and worker identity never produce a partial period PDF', async () => {
  for (const option of [{ badHash: true }, { badWorker: true }]) {
    const c = periodFixture(option); await c.mount().refresh(); await button(c.host, 'Download period PDF').click();
    assert.equal(c.objectUrls.length, 0); assert.equal(c.workers[0].terminated, true);
    assert.equal(c.calls.some(row => row.body.operation === 'analytics_pdf_prepared'), false);
  }
});

test('an impossible whole-report page minimum refuses before worker allocation or member reads', async () => {
  const c = periodFixture(); c.data.scope.manifest = Array.from({ length: 20 }, (_, i) => ({ ...c.data.scope.manifest[0], attemptId: String(i) }));
  c.data.scope.analytics.completedAttempts = 20;
  await c.mount().refresh(); await button(c.host, 'Download period PDF').click();
  assert.equal(c.workers.length, 0); assert.equal(c.calls.some(row => row.body.operation === 'analytics_attempt'), false);
  assert.match(c.host.textContent, /narrower reporting period/);
});
test('attachment-free scope email has a finite deadline and keeps its scope without auto resend', async () => {
  ...
  const timer = [...c.timers.values()].find((row) => row.delay === 25000); assert.ok(timer); timer.fn(); await emailing;
...
test('cancelling an in-flight scope email stops waiting without claiming that delivery was cancelled', async () => {
  const c = fixture(); let finish;
  c.transport((body) => body.operation === 'history' ? payload() : body.operation === 'analytics_snapshot' ? scopePayload()
    : new Promise(resolve => { finish = resolve; }));
  await c.mount().refresh(); const emailing = button(c.host, 'Email period report').click(); await flush();
  assert.equal(c.calls.filter(row => row.body.operation === 'analytics_email').length, 1);
  const scopeUrl = c.location.href;
  await button(c.host, 'Cancel').click(); await emailing;
  assert.match(c.host.textContent, /Stopped waiting for email status/);
  assert.match(c.host.textContent, /Sending may still finish; check your inbox/);
  assert.doesNotMatch(c.host.textContent, /Action cancelled/);
  finish({ email: { status: 'provider_accepted' } }); await flush();
  assert.match(c.host.textContent, /Stopped waiting/); assert.equal(c.location.href, scopeUrl);
  assert.equal(c.calls.filter(row => row.body.operation === 'analytics_snapshot').length, 1);
  assert.equal(c.calls.filter(row => row.body.operation === 'analytics_email').length, 1);
});
...
test('Google and session return preserve only one exact Forecast scope/attempt UUID on its known local route', async () => {
  ...
  assert.ok(/phase2-experience\.js\?[^"\n]*analytics=astra-analytics-browser-20260908-r1/u.test(html));
});
