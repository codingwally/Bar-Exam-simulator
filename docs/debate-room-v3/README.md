...
The owner explicitly authorized public access for every signed-in member, paid or unpaid, with a separate Debate Room link beside Study Room. The earlier admin-only preview interpretation is superseded. Publication and full acceptance remain distinct: the owner requested access while implementation continues. [Publication requirements and current evidence](publication-20260910.md) track the public entry, professional copy, desktop layout and remaining rehearsal gaps. [Production database evidence](evidence/production-database-20260910.json) verifies both additive migrations, preserved Study records, rollback-only checks, permissions and private bucket configuration. [Exact `44451be` Worker and Pages publication is recorded](evidence/publication-34401062163.json), including 29 live asset hashes. [Actual returning Chrome still reused old room scripts](evidence/live-browser-cache-gap-44451be.json) and displayed the timer error, so corrected live lobby behavior is not yet confirmed. Copy/layout source and controlled-browser checks passed; full hosted rehearsal, private files, physical media, real mail and separate paid/unpaid journeys remain open.
