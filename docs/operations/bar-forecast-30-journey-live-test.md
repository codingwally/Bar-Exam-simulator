...
- The runner creates only temporary `admin` and non-admin test identities bearing the `dd-forecast-e2e-` prefix. It globally signs out each browser session, deletes consent and role rows, deletes each Auth user, and verifies cleanup before reporting success.
...
If `cleanup.complete` is false, immediately revoke the temporary key and perform manual cleanup: remove Auth users whose email begins with the run's `dd-forecast-e2e-<run-id>-` prefix, remove their `user_roles` rows, and remove their `dd2026_bar_forecast_consents` rows. Verify all three are empty for those user IDs before retrying. Never delete by a broad wildcard or touch non-test users.
