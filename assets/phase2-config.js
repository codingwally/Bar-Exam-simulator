...
(function maintenancePasswordGate(global) {
  ...
  if (!maintenance?.enabled || !config?.workerUrl || global.__ddMaintenanceGateInstalled) return;
  ...
  function saveToken(token) {
    try {
      ...
      // A private browser may deny persistent storage; the current page still works.
    }
  }
  ...
  global.fetch = function maintenanceAwareFetch(input, init = {}) {
    const url = requestUrl(input);
    if (!url || url.origin !== workerOrigin) return nativeFetch(input, init);
