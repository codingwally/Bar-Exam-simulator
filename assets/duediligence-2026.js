(function dueDiligence2026Experience(global) {
  ...
  function examEntry() {
    ...
    const cards = [
      ['professor', 'Professor', 'Make the examination', 'Choose the number of questions, upload them, check them, set the rules, publish, and grade.'],
      ['beadle', 'Beadle', 'Upload students and prepare exam day', 'Upload the class list, check who may enter, and help students during the examination.'],
      ['student', 'Student', 'Take the examination', 'Sign in, check the exam details, answer each question, review, and submit.'],
      ['admin', 'Admin', 'Open the Admin area', 'Check Examination Room status and handle authorized school support.'],
    ];
  ...
  function examSection(portal) {
    ...
    if (state.exam.section === 'professor') return portal.roles?.professor ? professorSection(portal) : activationSection();
  ...
  function professorSection(portal) {
    ...
    return `<section class="dd26-card"><div class="dd26-label">Professor workspace</div><h2>Make a class examination</h2><p>1 Make the exam → 2 Check every question → 3 Set the exam rules → 4 Publish. After a student starts, any correction is shown as a clear notice instead of silently changing the exam.</p><div class="dd26-form-grid"><label class="dd26-field"><span>Class title</span><input class="dd26-input" id="dd26-class-title" maxlength="200" placeholder="Evidence · Section A"></label><label class="dd26-field"><span>School</span><input class="dd26-input" id="dd26-class-school" maxlength="300"></label><label class="dd26-field"><span>Academic term</span><input class="dd26-input" id="dd26-class-term" maxlength="160"></label></div><div class="dd26-actions"><button class="dd26-button primary" id="dd26-create-class" type="button">Create class</button></div></section>${classes.length ? `<section class="dd26-card"><div class="dd26-toolbar">${classes.map((entry) => `<button class="dd26-chip${entry.classroomId === activeClass?.classroomId ? ' is-active' : ''}" type="button" data-dd26-class="${escapeHtml(entry.classroomId)}" ${entry.classroomId === activeClass?.classroomId ? 'aria-pressed="true"' : 'aria-pressed="false"'}>${escapeHtml(entry.title)}</button>`).join('')}</div>${activeClass ? professorClass(activeClass) : ''}</section>` : '<section class="dd26-card"><div class="dd26-empty">Create your first class, then make its examination.</div></section>'}`;
  }

  function professorClass(classroom) {
    return `<div class="dd26-question-meta"><span>${escapeHtml(classroom.title)}</span><span class="dd26-status">${classroom.rosterCount || 0} students on the class list</span></div><div class="dd26-notice"><strong>The Beadle uploads the students.</strong> After you create the exam, invite the Beadle from the exam card to upload and check the class list.</div><section class="dd26-section"><div class="dd26-stepper" aria-label="Professor exam steps"><span class="is-active">1 Make exam</span><span>2 Check questions</span><span>3 Set exam rules</span><span>4 Publish</span></div><h3>Make an examination</h3><div class="dd26-form-grid"><label class="dd26-field"><span>Exam title</span><input class="dd26-input" id="dd26-exam-title" maxlength="200"></label><label class="dd26-field"><span>Number of questions</span><input class="dd26-input" id="dd26-exam-count" type="number" min="1" max="200" step="1"><small class="dd26-help">Choose 1–200 questions for this beta. The Professor decides the number.</small></label><label class="dd26-field wide"><span>Instructions for students</span><textarea class="dd26-textarea" id="dd26-exam-instructions" maxlength="10000"></textarea></label><label class="dd26-field"><span>If a student leaves the exam tab</span><select class="dd26-select" id="dd26-exam-integrity"><option value="standard" selected>Record for Professor review</option><option value="strict">Warn the student and record</option></select><small class="dd26-help">Copy, cut, and paste are blocked during the monitored exam. Leaving the tab never causes an automatic failure.</small></label><label class="dd26-field"><span>What students receive with grades</span><select class="dd26-select" id="dd26-exam-questionnaire"><option value="false">Grades and comments only</option><option value="true">Include the questionnaire</option></select></label></div><div class="dd26-actions"><button class="dd26-button primary" id="dd26-create-exam" type="button">Create exam draft</button></div></section><section class="dd26-section"><h3>Your examinations</h3>${examCards(classroom.exams || [])}</section>`;
  }
