...
test('receipt modes belong to the actual application Workers, not the forwarding public API alias', () => {
  const production = readFileSync(new URL('./wrangler.toml', import.meta.url), 'utf8');
  const staging = readFileSync(new URL('./wrangler.staging.toml', import.meta.url), 'utf8');
  const alias = readFileSync(new URL('./wrangler.public-api.toml', import.meta.url), 'utf8');
  const maintenanceEntry = readFileSync(new URL('./maintenance-entry.mjs', import.meta.url), 'utf8');
  const aliasEntry = readFileSync(new URL('./public-api-alias.mjs', import.meta.url), 'utf8');
  const release = readFileSync(new URL('../.github/workflows/release-unlimited-feature-access.yml', import.meta.url), 'utf8');
  assert.match(production, /^name = "duediligence-gemini-examiner"$/m);
  assert.match(production, /^main = "maintenance-entry\.mjs"$/m);
  assert.match(maintenanceEntry, /import applicationWorker from '\.\/commercial-entry\.mjs'/);
  assert.match(staging, /^main = "commercial-entry\.mjs"$/m);
  assert.match(alias, /^main = "public-api-alias\.mjs"$/m);
  assert.match(alias, /^binding = "DUE_DILIGENCE_APPLICATION"$/m);
  assert.match(alias, /^service = "duediligence-gemini-examiner"$/m);
  assert.doesNotMatch(alias, /SUBSCRIPTION_RECEIPT_EMAIL_MODE|RESEND_API_KEY/);
  assert.match(aliasEntry, /env\?\.DUE_DILIGENCE_APPLICATION/);
  assert.match(aliasEntry, /await application\.fetch\(request\)/);
  assert.match(release, /Deploy Worker before exposing the matching Pages client[\s\S]*?workingDirectory: worker[\s\S]*?command: deploy\r?\n/);
  assert.match(release, /command: deploy --config wrangler\.staging\.toml/);
  assert.match(release, /command: deploy --config wrangler\.public-api\.toml/);
  const validation = readFileSync(new URL('../.github/workflows/validate-mandatory-early-access.yml', import.meta.url), 'utf8');
  const approvedScope = release.slice(release.indexOf('          approved_scope='), release.indexOf('          actual_scope='));
  for (const file of ['worker/subscription-receipt.mjs', 'worker/subscription-receipt.test.mjs', 'worker/astra-subscription-receipt-policy.test.mjs']) {
    assert.ok(approvedScope.includes(`            ${file} \\`), `Missing exact release path: ${file}`);
    assert.ok(validation.includes(`- '${file}'`), `Missing mandatory validation trigger: ${file}`);
  }
});
