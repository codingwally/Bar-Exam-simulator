...
const migrationPath = 'supabase/migrations/20260907230740_astra_staging_payment_fixture_registration.sql';
...
const identity = (label='student') => ({label,runId:RUN,
  email:`dd-complete-beta-${label}-${RUN}@duediligence.ph`,
  displayName:`Release ${label}`,source:`astra_complete_beta_staging_v1:${RUN}:${label}`});
...
async function seed(db,who=identity()) {
  await db.query('insert into auth.users(id,email,raw_app_meta_data,raw_user_meta_data) values($1,$2,$3,$4)',
    [ID,who.email,{provider:'email',providers:['email'],astra_staging_complete_beta_fixture:{version:1,runId:who.runId,label:who.label}},
...
async function register(db,{id=ID,runId=RUN,label='student',schema='public',role='service_role'}={}) {
  ...
  return (await call(db,`select ${schema}.astra_register_staging_payment_fixture($1::uuid,$2::text,$3::text) v`,
...
test('optional complete-beta registrar uses actual SQL, preserves account state, and fails closed',async t=>{
  ...
  try {
    await t.test('service-only invoker wrapper/private definer and postgres-only registry',async()=>{
      const rows=(await db.query(`select n.nspname,p.prosecdef,p.proconfig,pg_get_userbyid(p.proowner) owner,
        ...
        where p.proname='astra_register_staging_payment_fixture' order by n.nspname`)).rows;
    ...
    for(const who of [identity('student'),identity('peer')]) {
      await t.test(`${who.label}: register as student before later Beta, preserve exact replay`,()=>fixtureCase(db,async()=>{
        ...
        assert.deepEqual(result,{registered:true,fixtureUserId:ID,dataScope:'internal_test',registrationVersion:'astra-staging-complete-beta-v1',replayed:false});
    ...
    const negatives=[
      ...
      ['NULL label',null,{label:null}],['unknown label',null,{label:'owner'}],['wrong allowed label',null,{label:'peer'}],
      ['Examination label',null,{label:'student-a'}],['admin label',null,{label:'admin'}],
      ['no trusted marker',"update auth.users set raw_app_meta_data=raw_app_meta_data-'astra_staging_complete_beta_fixture'"],
      ['editable marker only',"update auth.users set raw_user_meta_data=raw_user_meta_data||jsonb_build_object('astra_staging_complete_beta_fixture',raw_app_meta_data->'astra_staging_complete_beta_fixture'),raw_app_meta_data=raw_app_meta_data-'astra_staging_complete_beta_fixture'"],
      ...[['version','2'],['runId','"different"'],['label','"admin"']].map(([key,value])=>[`different marker ${key}`,`update auth.users set raw_app_meta_data=jsonb_set(raw_app_meta_data,'{astra_staging_complete_beta_fixture,${key}}','${value}')`]),
      ['extra marker property',"update auth.users set raw_app_meta_data=jsonb_set(raw_app_meta_data,'{astra_staging_complete_beta_fixture,extra}','true')"],
    ...
    for(const [name,sql,args] of negatives) await t.test(`deny ${name} before any registry write`,()=>fixtureCase(db,async()=>{
      ...
      await assert.rejects(()=>register(db,args),error=>error.code==='P0001'&&/^Staging complete-beta fixture /.test(error.message));
    ...
    for(const [name,change] of [
      ['private body',"create or replace function private.astra_register_staging_payment_fixture(p_user_id uuid,p_run_id text,p_label text) returns jsonb language sql security definer set search_path='' as $$ select '{}'::jsonb $$"],
      ['public ACL','grant execute on function public.astra_register_staging_payment_fixture(uuid,text,text) to authenticated'],
      ['wrapper volatility','alter function public.astra_register_staging_payment_fixture(uuid,text,text) stable'],
      ['wrapper strict NULL bypass','alter function public.astra_register_staging_payment_fixture(uuid,text,text) strict'],
      ['private leakproof','alter function private.astra_register_staging_payment_fixture(uuid,text,text) leakproof'],
      ['wrapper parallel safety','alter function public.astra_register_staging_payment_fixture(uuid,text,text) parallel safe'],
      ...
      await db.exec('begin');try{
        ...
        const sql="select n.nspname,pg_get_functiondef(p.oid) body,p.proacl::text acl from pg_proc p join pg_namespace n on n.oid=p.pronamespace where p.proname='astra_register_staging_payment_fixture' order by n.nspname";
        const before=(await db.query(sql)).rows;await db.exec('savepoint migration_attempt');
        await assert.rejects(()=>db.exec(migration),/Staging complete-beta (registration (function collision or drift|prerequisite missing)|registry protection changed)/);
