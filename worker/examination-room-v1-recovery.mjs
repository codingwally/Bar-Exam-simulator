...
const SAFE_ERRORS = Object.freeze({
  ...
  [RECOVERY_ERROR_CODES.STORAGE_UNAVAILABLE]: [
    'The private Examination Room recovery bucket is temporarily unavailable.',
    503,
    'No database records were changed. Check the R2 binding and bucket status, then run Preflight again.',
  ],
...
function r2Binding(env, operations) {
  const binding = env?.EXAMINATION_ROOM_BACKUPS;
  if (!binding || operations.some((operation) => typeof binding[operation] !== 'function')) {
    throw recoveryError(RECOVERY_ERROR_CODES.NOT_CONFIGURED);
  }
  return binding;
}

function r2MetadataMatches(object, expected) {
...
function descriptorFrom(object, expected, duplicate = false) {
  const stored = object?.customMetadata || {};
  ...
  return Object.freeze({
    format: FORMAT,
    encryptedObjectReference: `r2:EXAMINATION_ROOM_BACKUPS:${expected.objectKey}`,
...
export function createExaminationRoomRecovery(dependencies = {}) {
  ...
  async function preflight(env) {
    ...
    const binding = r2Binding(env, ['head', 'get', 'put']);
    try {
      await binding.head(`${OBJECT_PREFIX}/.preflight`);
    } catch (cause) {
  ...
  async function materialize(env, request) {
    ...
    const binding = r2Binding(env, ['head', 'put']);
    ...
    if (existing) {
      if (!r2MetadataMatches(existing, expected)) throw recoveryError(RECOVERY_ERROR_CODES.OBJECT_CONFLICT);
      return descriptorFrom(existing, expected, true);
    }
    ...
    if (!stored) {
      ...
      if (!raced || !r2MetadataMatches(raced, expected)) {
        throw recoveryError(RECOVERY_ERROR_CODES.OBJECT_CONFLICT);
      }
      return descriptorFrom(raced, expected, true);
    }
    return descriptorFrom(stored, expected, false);
  }

  async function head(env, input) {
    const reference = expectedReference(input);
    const binding = r2Binding(env, ['head']);
    let object;
    try {
  ...
  async function retrieve(env, input) {
    const reference = expectedReference(input);
    const binding = r2Binding(env, ['get']);
    let object;
    try {
    ...
    return Object.freeze({
      ...
      descriptor: Object.freeze({
        format: FORMAT,
        encryptedObjectReference: `r2:EXAMINATION_ROOM_BACKUPS:${reference.objectKey}`,
        objectKey: reference.objectKey,
