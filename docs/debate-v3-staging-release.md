...
This package implements the owner's authorized controlled testing and restricted preview in draft PR 356. It does not authorize public launch. Both reviewed additive staging database migrations and the bounded hosted rollback/privilege checks have passed; see [the actual database receipt](debate-room-v3/evidence/staging-database-20260909.json). Read-only Cloudflare captures have run. The manual Worker deployment and real Auth/organizer journey have not run successfully; unit fixtures and synthetic browser results do not establish those outcomes.
