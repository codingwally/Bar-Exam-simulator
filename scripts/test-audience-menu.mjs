...
assert.match(html, /\.nav-audience-cluster\{[^}]*position:absolute[^}]*left:50%[^}]*transform:translateX\(-50%\)[^}]*display:flex[^}]*justify-content:center[^}]*gap:24px[^}]*width:max-content/);
assert.match(html, /@media \(max-width:1120px\) and \(min-width:701px\)\{[\s\S]*?\.nav-audience-cluster\{[^}]*position:relative[^}]*left:auto[^}]*transform:none[^}]*justify-content:center[^}]*width:100%[^}]*flex:1 1 100%/);
...
assert.match(html, /\.nav-group-academy \.nav-group-label\{[^}]*color:#F6EFDD[^}]*linear-gradient\(120deg,#002147,#0E3A67 45%,#6E97BE 60%,#002147\)[^}]*background-size:220% 100%[^}]*animation:sheen 3\.2s linear infinite/);
assert.match(html, /\.nav-group-academy \.spa-tab\{[^}]*border-color:rgba\(91,139,187,\.52\)/);
assert.doesNotMatch(html, /\.nav-group-academy \.nav-group-label\{[^}]*background:#4A154B/);
