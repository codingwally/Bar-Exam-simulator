...
test('legacy open student tabs can upload their local answer snapshot during submit', async () => {
  ...
  const response = await handlers.studentCommand(
    makeRequest('/examination-room/v1/student/command', {
      ...
      payload: {
        sessionId: IDS.session,
        sessionToken: SESSION_TOKEN,
        clientCompletedAt: '2026-08-26T03:59:55.000Z',
        answers: [{
          questionId: 'q001',
  ...
  assert.ok(
    calls.indexOf(intentCall) < calls.indexOf(saveCall),
    'submission intent must be recorded before final answer reconciliation',
  );
  assert.equal(intentCall.payload.clientCompletedAt, '2026-08-26T03:59:55.000Z');
  assert.equal(saveCall.payload.answerRevision.questionKey, 'q001');
  ...
  assert.equal(saveCall.payload.savedAt, '2026-08-26T03:59:55.000Z');
  ...
  assert.equal(submitCall.payload.submissionManifest.submittedAt, '2026-08-26T03:59:55.000Z');
  assert.equal(submitCall.payload.submissionManifest.questionCount, 1);
...
test('student submit persists blank answers and still produces a receipt', async () => {
  ...
  const saveCall = calls.find((entry) => entry.operation === 'save_answer');
  const submitCall = calls.find((entry) => entry.operation === 'submit');
  assert.ok(saveCall, 'unanswered question must still receive a final server revision');
