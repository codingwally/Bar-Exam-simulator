...
-- Run only after both September pricing prerequisites and the exact Forecast
-- migration are present in the Supabase migration ledger.
begin;
...
begin
  ...
  if v_content_type_constraint is null
     ...
     or position('2026.3' in v_source_version_constraint) = 0 then
  ...
  if not (
    ...
    raise exception 'ADMIN_BAR_FORECAST_PROBE_RPC_SECURITY_FAILED';
  end if;
end;
