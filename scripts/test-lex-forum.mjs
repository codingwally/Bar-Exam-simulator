...
assert.doesNotMatch(
  forum,
  /textElement\('h3',\s*'quorum-entry-heading'/,
  'Home posts must render as one human-written body without a separate title line.',
);
assert.doesNotMatch(css, /\.quorum-entry-heading\s*\{/);
...
await import('./test-home-display.mjs');
