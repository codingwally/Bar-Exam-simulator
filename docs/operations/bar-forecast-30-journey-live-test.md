...
The rate plan is fail-closed. Each examiner can issue four setup probes plus at most six counted Forecast requests. At a 180-second cadence, no ten-minute window can contain more than four two-examiner batches. Including the one non-admin denial probe, the planned maximum is 81 requests against the network limit of 180, leaving 99 requests of network headroom. Because every journey uses a distinct verified identity, the maximum is ten requests per account against the per-user limit of 30, leaving 20 requests of verified-user headroom. Evidence must independently reproduce the observed batch spacing and both limits and headroom values.
...
- The runner creates only 30 temporary `admin` identities and one unpaid non-admin identity bearing the `dd-forecast-e2e-` prefix. It verifies the denial identity has no subscription or Founding Beta record, globally signs out each browser session, deletes consent and role rows, removes that exact user's `usage_events` and then `usage_sessions`, deletes each Auth user, and verifies every scoped row is gone before reporting success. Success requires exact cleanup of all 31 accounts.
...
A passing JSON artifact must show `journeysRequested: 30`, `journeysPassed: 30`, `batchesPassed: 15`, exactly two active contexts, observed batch spacing of at least 180 seconds, 30 distinct administrator identities used once each, 30 required-setup boundary proofs, 30 post-setup authorized-status proofs, no more than six approved counted Forecast requests in their exact order per journey, five journeys for every subject, `nonAdministratorDenied: true` for the verified unpaid fixture, at least 99 requests of observed network headroom, at least 20 requests of observed verified-user headroom, and exact cleanup of all 31 accounts. The `releaseSha` and `githubRunId` must match the reviewed workflow candidate.
