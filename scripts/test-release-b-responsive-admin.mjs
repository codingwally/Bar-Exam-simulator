...
assert.match(admin, /mount\.classList\.add\('row-actions', 'row-actions-visible'\)/);
assert.doesNotMatch(admin, /menu\.className = 'action-menu'/);
assert.match(admin, /setAttribute\('aria-busy', 'true'\)/);
assert.match(admin, /refreshButton\.innerHTML = '[^']*Loading…[^']*'/);
...
console.log('Release B responsive navigation, admin drilldown, and visible-action contracts passed.');
